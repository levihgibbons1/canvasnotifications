...
app.listen(config.port, () => {
  ...
  console.log(`  OAuth: ${config.canvasOAuth.enabled ? 'configured' : 'not configured (token + demo login available)'} · SMTP: ${config.smtp.enabled ? 'on' : 'simulated'} · SMS: ${config.twilio.enabled ? 'on' : 'simulated'}`);
});
