...
function normalizeBase(raw: string): string | null {
  ...
  try { const p = new URL(u); return p.host.includes('.') ? `${p.protocol}//${p.host}` : null; } catch { return null; }
}
...
export default function Login() {
  ...
  return (
    <div className="min-h-screen grid lg:grid-cols-[1fr_1.1fr]">
      <section className="relative overflow-hidden px-8 py-10 lg:px-14 lg:py-16 flex flex-col justify-between border-b lg:border-b-0 lg:border-r border-line">
        ...
        <div className="relative mt-14 lg:mt-0 max-w-xl">
          ...
          <p className="mt-6 text-lg text-ink-2 leading-relaxed rise" style={{ animationDelay: '160ms' }}>
          ...
          <ul className="mt-8 grid sm:grid-cols-2 gap-x-8 gap-y-3 text-sm text-ink-2 rise" style={{ animationDelay: '240ms' }}>
            {[
...
function Intro({ children }: { children: ReactNode }) {
  return <p className="text-sm text-ink-2 leading-relaxed mb-5">{children}</p>;
}
